            ...
            news.forEach(item => {
                ...
                card.innerHTML = `
                    <div class="news-card-header">
                        ...
                        <span class="news-date">${item.date_fetched || "2026-08-20"}</span>
                    </div>
...
async function loadAutopilotFeed() {
    ...
    if (autoVol) autoVol.textContent = "Calculating...";
    if (autoTrucks) autoTrucks.textContent = "Calculating...";
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (autoVol) autoVol.innerHTML = `${data.total_volume_ton.toLocaleString('en-US')} <span class="unit">Tons</span>`;
            if (autoTrucks) autoTrucks.innerHTML = `${data.total_trucks.toLocaleString('en-US')} <span class="unit">Trucks (15T)</span>`;
